...
export function ResultView({ id }: { id: string }) {
  ...
  return <main className="result-page"><header className="radar-header radar-gutter"><a className="radar-wordmark" href="/">PreStocks <span>Radar</span></a><a className="result-header-link" href="/#demand">Explore demand ↗</a></header>
    <section className="result-main radar-gutter"><div className="result-intro"><p>One wallet. One current signal.</p><h1>My signal, in full.</h1><p>This link reflects the latest allocation from the wallet that published it.</p></div>
      ...
      {result && <div className="result-actions"><button className="radar-button dark" onClick={() => void copy()}>{copied ? "Link copied" : "Copy share link"}<span aria-hidden="true">↗</span></button><a className="radar-text-link" href="/#demand">Explore demand board ↗</a></div>}
